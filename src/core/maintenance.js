...
export function control(db) {
  ...
  const downtimes = db.prepare("SELECT d.*, e.folio AS equipment_folio, e.name AS equipment_name, e.downtime_cost_per_hour, ROUND(d.duration_minutes / 60.0 * e.downtime_cost_per_hour, 2) AS downtime_cost, o.folio AS order_folio FROM maintenance_downtimes d JOIN maintenance_equipment e ON e.id = d.equipment_id LEFT JOIN maintenance_orders o ON o.id = d.order_id ORDER BY d.id DESC LIMIT 100").all();
